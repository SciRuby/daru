      ...
      context "#set" do
        context Daru::Index do
          ...
          context "add" do
            ...
            let(:dv) { Daru::Vector.new [1, 2, 3], index: idx }
            
            context "single index" do 
              subject { dv }
              before { dv.set [:d], 'x' }
              
              its(:to_a) { is_expected.to eq [1, 2, 3, 'x'] }
              its(:'index.to_a') { is_expected.to eq [:a, :b, :c, :d] }
            end
            ...
            context "multiple indexes" do
              ...
              its(:to_a) { is_expected.to eq [1, 2, 3, 'x', 'x'] }
        ...
        context Daru::CategoricalIndex do
          ...
          context "add" do
            ...
            let(:dv) { Daru::Vector.new [1, 2, 3, 4, 5], index: idx }
            
            context "single index" do
              subject { dv }
              before { dv.set [:d], 'x' }
              
              its(:to_a) { is_expected.to eq [1, 2, 3, 4, 5, 'x'] }
              its(:'index.to_a') { is_expected.to eq [:a, 1, :a, 1, :c, :d] }
            end
            ...
            context "multiple indexes" do
              ...
              its(:to_a) { is_expected.to eq [1, 2, 3, 4, 5, 'x', 'x'] }
              its(:'index.to_a') { is_expected.to eq [:a, 1, :a, 1, :c, :c, :d] }
            end
          end
        end
      end      
