...
describe DateTimeIndex do
  ...
  context ".date_range" do
    ...
    it "creates a DateTimeIndex of weekly frequency between start and end" do
      index = DateTimeIndex.date_range(start: '2014-8-2', end: '2014-9-8', 
        freq: :W)

      expect(index).to eq(DateTimeIndex.new([
        Time.new(2014,8,2) ,Time.new(2014,8,9),Time.new(2014,8,16),
        Time.new(2014,8,23),Time.new(2014,8,30),Time.new(2014,9,6)]))
      expect(index.frequency).to eq(:W)
    end

    it "creates a DateTimeIndex of yearly frequency between start and end" do
      index = DateTimeIndex.date_range(start: '2014-9',end: '2018-1',freq: :Y)

      expect(index).to eq(DateTimeIndex.new([
        Time.new(2014,9), Time.new(2015,9),Time.new(2016,9), Time.new(2017,9)]))
      expect(index.frequency).to eq(:Y)
    end
    ...
    it "creates only specfied number of periods taking precendence over end" do
      index = DateTimeIndex.date_range(start: '2014-5-5', end: '2015-3', 
        periods: 5, freq: :Y)
      expect(index).to eq(DateTimeIndex.new([
        Time.new(2014,5,5),Time.new(2015,5,5),Time.new(2016,5,5),
        Time.new(2017,5,5),Time.new(2018,5,5)]))
    end
  end
  ...
  context "#[]" do
    ...
    it "accepts year and month specified as a string" do
      ...
      index = DateTimeIndex.date_range(start: '2014-1-1', periods: 100, freq: :M)
    ...
    it "accepts year, month, date and specific time as a string" do
      index = DateTimeIndex.date_range(
        :start => Time.new(2015,5,3),:end => Time.new(2015,5,5), freq: :Min)
